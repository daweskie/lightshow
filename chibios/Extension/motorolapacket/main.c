...
/*
Motorola config variables

*/

MotorolaPacketDriver MOTD1;
...
const uint8_t maxPackets=10;
const uint8_t preambleCounts=2;
uint8_t data[MTU];

msg_t MotorolaTestThread(void *p) {
    BaseSequentialStream *ch=p;
    chprintf(ch, "Beginning of Motorla test");

    int i=0;
    for (;i<MTU;i++)
        data[i]=i;

    motorolaObjectInit(&MOTD1, "mot0", NULL,
                       maxPackets, MTU, preambleCounts);

    chprintf(ch, "Object initialized\r\n");

    spStart(&MOTD1,&serialConfig);

    chprintf(ch, "Motorola device started\r\n");
    // It has to be empty

    assert(spIsEmpty(&MOTD1));
    chprintf(ch, "Queue is empty\r\n");

    // And no packets
    assert(!spGetNextPacket(&MOTD1));
    chprintf(ch, "No next packets\r\n");

    for (i=0;i<10;i++) {
        assert(motSend(&MOTD1,11,12,MTU,data)==EXIT_SUCCESS);
        chprintf(ch, "One packet sent\r\n");
        chThdSleep(10);
    }

    for (i=0;i<10;i++) {
        chprintf(ch, "%d. nextPacket\r\n",i);
        struct SerialPacketItem *item = spGetNextPacket(&MOTD1);
        assert(item);
        assert(item->valid);
        assert(item->address==11);
        assert(item->command==12);
        assert(item->len == MTU);
        assert(item->data);
        int j=0;
        for (j=0;j<MTU;j++)
            assert(*(item->data+j) == j);
        chprintf(ch, "%d. recyclePacket\r\n",i);
        spRecyclePacket(&MOTD1, item);
    }

    struct SerialPacketItem *item = spGetNextPacket(&MOTD1);
    assert(!item);
    chprintf(ch, "OK, there are no remaining packets\r\n",i);
    return (msg_t) TRUE;
}
