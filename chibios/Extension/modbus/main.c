...
uint16_t registers[MAX_REGS];

msg_t ModbusTestThread(void *p) {
    BaseSequentialStream *ch=p;
    chprintf(ch, "Beginning of Modbus test");

    int i=0;
    for (;i<MTU;i++)
        data[i]=i;

    modbusObjectInit(&MODD1, NULL,
                       maxPackets, MTU, MODBUS_ADDRESS, &registers, MAX_REGS, MODBUS_REGISTER_BASE);

    chprintf(ch, "Object initialized\r\n");

    spStart(&MODD1,&serialConfig);

    chprintf(ch, "Modbus device started\r\n");
    // It has to be empty

    assert(spIsEmpty(&MODD1));
    chprintf(ch, "Queue is empty\r\n");

    // And no packets
    assert(!spGetNextPacket(&MODD1));
